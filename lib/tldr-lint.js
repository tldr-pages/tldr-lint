...
module.exports.ERRORS = parser.ERRORS = {
  ...
  'TLDR112': 'Translated lines must not be identical to English page lines'
};
...
linter.processFile = function(file, verbose, alsoFormat, ignoreErrors) {
  ...
  const pagesLangMatch = file.match(/\/pages\.([^\/]+)\//);
  if (pagesLangMatch && pagesLangMatch[1] && pagesLangMatch[1] !== 'en') {
    const lang = pagesLangMatch[1];
    const englishPath = file.replace(new RegExp(`/pages\\.${lang}\\/`), '/pages/');
    if (fs.existsSync(englishPath)) {
      const englishContent = fs.readFileSync(englishPath, 'utf8').split(/\r?\n/).map(l => l.trim());
      const englishSet = new Set(englishContent);
      const translatedLines = fs.readFileSync(file, 'utf8').split(/\r?\n/);
      translatedLines.forEach(function(line, idx) {
        if (line.match(/^\s*[>-]/)) {
          if (englishSet.has(line.trim())) {
            result.errors.push({ locinfo: { first_line: (idx+1).toString() }, code: 'TLDR112', description: this.ERRORS.TLDR112 });
          }
        }
      }, this);
    }
