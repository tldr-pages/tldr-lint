...
(function(parser) {
  ...
  parser.yy.addMoreInformation = function(url) {
    parser.yy.page.moreInformation.push(url);
  };
...
linter.format = function(parsedPage) {
  ...
  parsedPage.description.forEach(function(moreInformation) {
    str += util.format('> More information: %s.', moreInformation);
    str += '\n';
  });
...
linter.process = function(page, verbose, alsoFormat) {
  ...
  if (verbose) {
    ...
    console.log(parser.yy.page.moreInformation.length + " link(s)");
  }
