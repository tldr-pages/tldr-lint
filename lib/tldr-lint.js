  ...
  if (pagesIdx !== -1 && parts[pagesIdx].slice(6) !== 'en') {
    ...
    if (fs.existsSync(englishPath)) {
      ...
      translatedLines.forEach(function(line, idx) {
        if (line.match(/^\s*[>-]/) && englishSet.has(line)) {
          const content = line.replace(/^[>-]\s/, '').trim();
          const words = content.split(/\s/).filter(w => w.length > 0);
          const moreThanTwoWords = words.length > 2;
          const everyWordCapitalized = words.length > 0 && words.every(word => /^[A-Z]/.test(word));
          if (moreThanTwoWords && !everyWordCapitalized) {
