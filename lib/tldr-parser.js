...
case 8:
  if (this.topState() === 'description') {
    ...
    // Check for hyperlinks that aren't enclosed in angle brackets
    var urlMatch = yy_.yytext.match(/https?:\/\/[^\s<>]+/);
    if (urlMatch) {
      var beforeUrl = yy_.yytext.substring(0, urlMatch.index);
      var afterUrl = yy_.yytext.substring(urlMatch.index + urlMatch[0].length);
      var urlBracketed = beforeUrl.endsWith('<') && afterUrl.startsWith('>');
      if (!urlBracketed) {
        yy.error(yy_.yylloc, 'TLDR113');
      }
    }
