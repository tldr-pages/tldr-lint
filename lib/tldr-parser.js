...
case 8:
  if (this.topState() === 'description') {
    ...
    var termMatch = yy_.yytext.match(/(?:^|[^`-\w])(stdout|stdin|stderr|regex)(?=[^`\w]|$)/i);
    if (termMatch) {
      ...
      if (!insideUrl) {
        var term = termMatch[1].toLowerCase();
        yy.errors.push({
          locinfo: yy_.yylloc,
          code: 'TLDR113',
          description: 'Term `' + term + '` should be lowercase and wrapped in backticks'
        });
      }
    }
